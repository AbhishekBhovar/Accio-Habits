...
function renderJourney(){
  ...
  const scenes={1:['🪞','🪄'],2:['🧪','🚙'],3:['🌳','🌘'],4:['🏆','🌿'],5:['🔮','🪶'],6:['⚗️','〰️'],7:['△⃝│','⚡']}[ui.journeyBook];
  const scenery=`<div class="book-watermark book-watermark-main book-${ui.journeyBook}">${scenes[0]}</div><div class="book-watermark book-watermark-small book-${ui.journeyBook}">${scenes[1]}</div>`;
  map.innerHTML=`<div class="map-sky"><div class="castle-mark">🏰</div><div><span>BOOK ${ui.journeyBook}</span><strong>${escapeHtml(BOOK_NAMES[ui.journeyBook])}</strong></div></div>${scenery}<svg class="map-path" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-hidden="true"><path d="${path}"/></svg>${nodes}<div class="map-footer-rune">MISCHIEF MANAGED • ${rows.filter(l=>l.level<=save.currentLevel).length}/24 LEVELS</div>`;
...
function collectionCardHtml(c){const status=cardStatus(c),owned=status==='owned',name=owned?visibleName(c):(status==='eligible'?'Undiscovered':'?'),meta=CATEGORY_META[c.category],art=owned?meta.icon:'✦';if(!owned)return `<article class="museum-card ${status} ${c.rarity}"><div class="museum-art"><span>${art}</span><i>?</i></div><div class="museum-copy"><span class="rarity ${c.rarity}">${c.rarity}</span><h4>${escapeHtml(name)}</h4><p>${status==='eligible'?`Eligible since Lv ${c.firstEligibleLevel}`:'Undiscovered'}</p></div></article>`;return `<button class="museum-card flip-card ${c.rarity}" data-flip-card="${escapeHtml(c.id)}" aria-label="Flip ${escapeHtml(name)} card"><span class="flip-card-inner"><span class="flip-face flip-front"><span class="museum-art"><span>${art}</span><i>${escapeHtml(name.slice(0,1))}</i></span><span class="museum-copy"><span class="rarity ${c.rarity}">${c.rarity}</span><h4>${escapeHtml(name)}</h4><p>Discovered Lv ${save.owned[c.id].discoveredLevel}</p><em>↻ Tap to flip</em></span></span><span class="flip-face flip-back"><span class="fact-spark">✦</span><small>STORY FACT</small><h4>${escapeHtml(name)}</h4><p>${escapeHtml(collectibleFact(c))}</p><em>↻ Tap to return</em></span></span></button>`;}
function bindCollectionCards(hub){hub.querySelectorAll('[data-flip-card]').forEach(card=>card.onclick=()=>{card.classList.toggle('flipped');playChime('success');});hub.querySelectorAll('[data-card]').forEach(el=>el.onclick=()=>{ui.collectionCard=el.dataset.card;ui.collectionMode='dossier';renderCollection();window.scrollTo({top:0,behavior:'smooth'});});}
function renderDossier(hub,c){const owned=save.owned[c.id],meta=CATEGORY_META[c.category],evolutions=(c.evolutionLevels||[]).filter(x=>Number(String(x).match(/\d+/)?.[0]||999)<=Math.max(1,save.currentLevel)),revelations=(c.revelationLevels||[]).filter(x=>Number(String(x).match(/\d+/)?.[0]||999)<=Math.max(1,save.currentLevel));hub.innerHTML=`<button id="collectionBack" class="back-button">← Magical Archive</button><section class="dossier"><div class="dossier-hero"><div class="dossier-icon">${meta.icon}</div><span class="rarity ${c.rarity}">${c.rarity} • ${escapeHtml(meta.title)}</span><h2>${escapeHtml(visibleName(c))}</h2><p class="muted">Discovered at Level ${owned.discoveredLevel}</p></div><div class="dossier-copy"><h3>WHAT YOU KNOW</h3><p>${escapeHtml(c.notes||`Your understanding of ${visibleName(c)} grows as the story reveals more. Future knowledge remains hidden until you reach it.`)}</p></div><div class="dossier-copy"><h3>STORY EVOLUTION</h3><div class="evolution-trail"><div class="evolution-step"><strong>Lv ${owned.discoveredLevel}</strong><span>Discovered</span></div>${evolutions.map(x=>`<div class="evolution-step"><strong>Lv ${escapeHtml(String(x))}</strong><span>New context</span></div>`).join('')}${revelations.map(x=>`<div class="evolution-step"><strong>Lv ${escapeHtml(String(x))}</strong><span>Revelation</span></div>`).join('')}</div></div></section>`;hub.querySelector('#collectionBack').onclick=()=>{ui.collectionCard=null;ui.collectionMode='home';renderCollection();};}
function renderCollection(){
  ...
  if(ui.collectionMode==='dossier'&&ui.collectionCard){const c=DATA.collectibles.find(x=>x.id===ui.collectionCard);if(c&&save.owned[c.id])return renderDossier(hub,c);ui.collectionMode='home';}
  if(ui.collectionMode==='memories'){
    const moments=ownedCards.filter(c=>c.category==='Moment');hub.innerHTML=`<button id="collectionBack" class="back-button">← Magical Archive</button><section class="archive-hero"><span class="eyebrow">YOUR STORY</span><div class="archive-count"><strong>Memory Timeline</strong></div><p>The moments you have lived through, in the order you discovered them.</p></section><div class="memory-timeline">${moments.length?moments.slice().reverse().map(c=>`<article class="memory-entry" data-card="${escapeHtml(c.id)}"><small>LEVEL ${save.owned[c.id].discoveredLevel}</small><h4>${escapeHtml(visibleName(c))}</h4><p>${escapeHtml(c.rarity)} memory</p></article>`).join(''):'<p class="muted">Your first memories will appear here as your journey unfolds.</p>'}</div>`;hub.querySelector('#collectionBack').onclick=()=>{ui.collectionMode='home';renderCollection();};bindCollectionCards(hub);return;
  }
  const recent=ownedCards.slice(0,5),cat=ui.collectionCategory,filtered=cat?DATA.collectibles.filter(c=>c.category===cat&&save.owned[c.id]):ownedCards;
  hub.innerHTML=`<section class="collection-overview"><div class="archive-hero"><span class="eyebrow">THE MAGICAL ARCHIVE</span><div class="archive-count"><strong>${ownedCards.length}</strong><span>/ 385 discovered</span></div><p>Every discovery becomes part of your world — and can reveal more as your story progresses.</p><div class="progress-track"><div class="progress-fill" style="width:${ownedCards.length/385*100}%"></div></div></div>${recent.length?`<div class="recent-section"><div class="section-head"><h3>✨ Latest discoveries</h3><span class="mini-badge">Tap a card</span></div><div class="recent-shelf">${recent.map(collectionCardHtml).join('')}</div></div>`:''}<div class="archive-actions"><button class="primary" id="browseArchive">🗃 Browse collection</button><button class="memory" id="memoryTimeline">🎞️ Memory timeline</button></div><section class="archive-browser" ${ui.collectionMode==='browse'?'':'hidden'}><div class="archive-browser-head"><h3>Browse your archive</h3><button id="closeBrowser">Close</button></div><div class="archive-chips"><button data-gallery="" class="${!cat?'active':''}">All ${ownedCards.length}</button>${Object.entries(CATEGORY_META).filter(([key])=>key!=='Moment').map(([key,m])=>{const n=ownedCards.filter(c=>c.category===key).length;return `<button data-gallery="${escapeHtml(key)}" class="${cat===key?'active':''}">${m.icon} ${m.title} ${n}</button>`;}).join('')}</div><div class="museum-grid">${filtered.length?filtered.map(collectionCardHtml).join(''):'<p class="muted">Nothing discovered here yet.</p>'}</div></section></section>`;
  hub.querySelector('#browseArchive').onclick=()=>{ui.collectionMode='browse';renderCollection();};hub.querySelector('#memoryTimeline').onclick=()=>{ui.collectionMode='memories';renderCollection();window.scrollTo({top:0,behavior:'smooth'});};const close=hub.querySelector('#closeBrowser');if(close)close.onclick=()=>{ui.collectionMode='home';ui.collectionCategory=null;renderCollection();};hub.querySelectorAll('[data-gallery]').forEach(b=>b.onclick=()=>{ui.collectionCategory=b.dataset.gallery||null;ui.collectionMode='browse';renderCollection();});bindCollectionCards(hub);
}
// ---------- Stats dashboard ----------
...
function sleepBarsHtml(values,maxHours){const labels=['M','T','W','T','F','S','S'];return `<div class="sleep-bars">${values.map((v,i)=>`<div class="sleep-bar-col"><i style="height:${Math.min(100,(Number(v)||0)/maxHours*100)}%"></i><small>${labels[i]}</small></div>`).join('')}</div>`;}
function renderStats(){
  const week=ensureCurrentWeek(), habits=habitWeekStats(), nutrition=habits.filter(nutritionHabit), foundations=habits.filter(h=>!nutritionHabit(h));
  const sportDone=3-week.sportDueRemaining,dates=currentWeekDates(),sleepRows=dates.map(k=>save.daily[k]?.sleep||{}),mainVals=sleepRows.map(x=>Number(x.mainHours)||0),napVals=sleepRows.map(x=>Number(x.napHours)||0),mainLogged=mainVals.filter(Boolean),napLogged=napVals.filter(Boolean),mainAvg=mainLogged.length?avg(mainLogged):0,napAvg=napLogged.length?avg(napLogged):0;
  document.querySelector('#statsDashboard').innerHTML=`
    ...
    <section class="dashboard-card"><div class="section-head"><div><span class="eyebrow">RECOVERY</span><h3>Sleep</h3></div></div><div class="sleep-split"><div class="sleep-graph-card"><div class="sleep-graph-title"><span>🌙 Night sleep</span><strong>${mainLogged.length?mainAvg.toFixed(1)+'h':'—'}</strong></div>${sleepBarsHtml(mainVals,10)}<small>7-day average</small></div><div class="sleep-graph-card"><div class="sleep-graph-title"><span>💤 Naps</span><strong>${napLogged.length?napAvg.toFixed(1)+'h':'—'}</strong></div>${sleepBarsHtml(napVals,3)}<small>7-day average</small></div></div></section>
    <div class="backup-inline"><span>Save & restore data</span><button id="exportSave">Export save</button><label>Import save<input id="importSave" type="file" accept="application/json" hidden></label></div>`;
  bindBackupControls();
}
function bindBackupControls(){const ex=document.querySelector('#exportSave'),im=document.querySelector('#importSave');if(ex)ex.onclick=()=>{const blob=new Blob([JSON.stringify(save,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`hp-fitness-rpg-save-${localDateKey()}.json`;a.click();URL.revokeObjectURL(a.href);};if(im)im.onchange=async e=>{const f=e.target.files[0];if(!f)return;try{save=mergeState(JSON.parse(await f.text()));persist();ensureCurrentWeek();finalizePastDays();render();}catch{alert('That save file could not be read.');}};}
