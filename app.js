...
function renderStats(){
  ...
  document.querySelector('#statsDashboard').innerHTML=`
    ...
    <section class="dashboard-card"><div class="section-head"><div><span class="eyebrow">RECOVERY</span><h3>Sleep</h3></div><strong>${ss.count?ss.sevenAvg.toFixed(1)+' h avg':'—'}</strong></div><div class="sleep-dashboard"><div class="sleep-ring" style="--pct:${Math.min(100,ss.sevenAvg/8*100)}"><span>${ss.count?ss.sevenAvg.toFixed(1)+'h':'—'}</span></div><div><p><span>Nights logged</span><strong>${ss.count}/7</strong></p><p><span>Recovery sleep</span><strong>${ss.recovery.toFixed(1)} h</strong></p><p><span>Shortfall vs 8h</span><strong>${ss.shortfall.toFixed(1)} h</strong></p></div></div></section>`;
}
