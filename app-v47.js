...
function renderJourney(){
  try{
    ...
    const currentBook=bookForLevel(current);
    const icons={
      1:`<svg viewBox="0 0 64 64"><circle cx="32" cy="32" r="7"/><path d="M25 29C16 21 9 20 5 21c5 8 11 12 21 15M39 29c9-8 16-9 20-8-5 8-11 12-21 15" fill="none"/></svg>`,
      2:`<svg viewBox="0 0 64 64"><path d="M25 8h14M28 8v15L16 47c-3 6 1 10 7 10h18c6 0 10-4 7-10L36 23V8M20 43h24" fill="none"/></svg>`,
      3:`<svg viewBox="0 0 64 64"><path d="M10 52C24 43 30 31 40 13M27 37l-10-8M34 28l11-8M21 44H10M39 20l8-10M42 16l11 2" fill="none"/></svg>`,
      4:`<svg viewBox="0 0 64 64"><path d="M18 10h28v9c0 11-6 18-14 18S18 30 18 19v-9zM32 37v12M22 54h20M18 14H9v7c0 6 4 10 10 10M46 14h9v7c0 6-4 10-10 10" fill="none"/></svg>`,
      5:`<svg viewBox="0 0 64 64"><path d="M8 39c11-3 16-9 21-20 4 8 10 13 19 13 4 0 7-1 9-2-5 10-14 16-24 17l-3 10-5-9c-8-1-13-4-17-9z" fill="currentColor" stroke="none"/><path d="M29 19l5-9 4 11" fill="none"/></svg>`,
      6:`<svg viewBox="0 0 64 64"><path d="M14 14l25-5 11 8v34l-25 5-11-8V14zM25 20l16-3M25 27l16-3M25 34l13-3M19 15v32l6 9" fill="none"/></svg>`,
      7:`<svg viewBox="0 0 64 64"><path d="M32 8L8 54h48L32 8zM32 8v46M19 35a13 13 0 1 0 26 0 13 13 0 1 0-26 0" fill="none"/></svg>`
    };
    const books=[1,2,3,4,5,6,7].map(n=>({n,start:(n-1)*24+1,end:n*24,icon:icons[n]}));
    const checkpoints=books.map((b,i)=>{
      const state=b.n<currentBook?'complete':b.n===currentBook?'current':'locked';
      return `<div class="peak-checkpoint cp-${b.n} ${state}">
        <div class="peak-icon">${b.icon}</div>
        <div class="peak-banner"><b>BOOK ${b.n}</b><small>Levels ${b.start}–${b.end}</small></div>
      </div>`;
    }).join('');
    const sagaPct=((current-1)/167)*100;
    root.innerHTML=`<section class="peak-map">
      <div class="peak-intro"><span>THE JOURNEY CONTINUES</span><h2>Climb. Conquer.<br>Become Legendary.</h2><div class="peak-level">Level ${current} <small>• Book ${currentBook}</small></div></div>
      <div class="peak-moon"></div><div class="peak-stars"></div>
      <div class="peak-range range-back"></div><div class="peak-range range-mid"></div>
      <div class="peak-main-mountain"></div><div class="peak-snow"></div>
      <div class="peak-castle" aria-label="Castle at the summit"><svg viewBox="0 0 220 145"><path d="M9 132h202M25 132V76h23V52l12-15 12 15v15h20V38l18-27 18 27v28h19V49l14-17 14 17v27h22v56H25z"/><path d="M100 132V99c0-12 20-12 20 0v33"/></svg></div>
      <div class="peak-trail"><div class="peak-trail-lit" style="height:${sagaPct}%"></div></div>
      ${checkpoints}
      <div class="peak-base"><span>BEGIN</span><b>168 levels • 7 books • one climb</b></div>
    </section>`;
  }catch(err){console.error('Journey render failed',err);const root=document.querySelector('#journeyContent');if(root)root.innerHTML='<div class="card" style="margin:24px"><h3>Journey map</h3><p class="muted">Your journey data is safe. The map could not render.</p></div>';}
}
