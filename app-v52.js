...
function renderJourney(){
  ...
  const yearPct=currentBook===1?Math.min(100,Math.round(Math.max(0,save.currentLevel)/24*100)):Math.min(100,Math.round((Math.max(25,save.currentLevel)-24)/24*100));
  const years=[
    ['The Threat Emerges','Foundation'],['The Chamber Opens','Current campaign'],['Fear & the Dementors','Locked'],['Voldemort Returns','Locked'],['The War Begins','Locked'],['The Horcrux Hunt','Locked'],['The Final Stand','Locked']
  ];
  const encounters=currentBook===1?[
    ['🧹','Flight Training','Aerial control and reactions'],['👹','Troll Attack','Magic becomes practical under pressure'],['🌲','Forbidden Forest','Courage and threat awareness'],['♟️','Trapdoor Trials','Skills, knowledge and allies combine'],['☠️','Quirrell','Year 1 boss battle']
  ]:[
    ['📖','Strange Attacks','Investigate the hidden threat'],['⚔️','Dueling Club','Combat magic and serpent speech'],['🕷️','Forest Encounter','Follow the evidence into danger'],['🚪','Enter the Chamber','Knowledge opens the path'],['🐍','Face the Basilisk','Year 2 boss battle'],['📕','Destroy the Diary','End the immediate threat']
  ];
  const doneCount=Math.floor(encounters.length*yearPct/100);
  root.innerHTML=`<section class="campaign-page"><span class="campaign-kicker">THE ROAD TO DEFEAT VOLDEMORT</span><h2>Campaign</h2><p class="campaign-sub">Seven years of preparation. Every trial builds toward the final battle.</p><div class="campaign-mountain"><div class="mountain-shape"></div><div class="summit-castle">🏰</div><div class="campaign-years">${years.map((y,i)=>{const n=i+1,top=82-i*12.2,cls=n<currentBook?'done':n===currentBook?'current':'locked';return `<div class="year-node ${cls}" style="top:${top}%"><b>YEAR ${n}</b><strong>${y[0]}</strong><small>${n===currentBook?'● ACTIVE':n<currentBook?'✓ COMPLETE':'🔒 '+y[1]}</small></div>`}).join('')}</div></div><section class="campaign-year-card"><span class="campaign-kicker">YEAR ${currentBook} CAMPAIGN</span><h3>${years[currentBook-1][0]}</h3><p class="muted">Only battle-relevant training, trials and confrontations appear here.</p><div class="campaign-progress"><i style="width:${yearPct}%"></i></div><small>${yearPct}% campaign progress</small><div class="encounter-list">${encounters.map((e,i)=>`<div class="encounter ${i<doneCount?'done':i===doneCount?'current':'locked'}"><span>${i<doneCount?'✓':e[0]}</span><div><b>${e[1]}</b><small>${i<doneCount?'Preparation secured':i===doneCount?e[2]:'Locked until preparation advances'}</small></div></div>`).join('')}</div></section></section>`;
}
...
function battleCategoryState(cat){const items=BATTLE_PROTO.items.filter(i=>i.cat===cat.id);return {items,unlocked:items.filter(battleUnlocked)};}
function readinessPct(){const all=BATTLE_PROTO.items, done=all.filter(battleUnlocked).length;return Math.round(done/all.length*100);}
...
function renderCollection(){
 ...
 hub.innerHTML=`<section class="readiness-hero-new"><div class="readiness-ring" style="--pct:${pct}"><div><strong>${pct}%</strong><span>${tier}</span></div></div><h2>Battle Readiness</h2><p>Build the magic, skill, intelligence and support Harry will need for the battles ahead.</p></section><section class="readiness-four">${BATTLE_PROTO.categories.map(cat=>{const s=battleCategoryState(cat),pc=Math.round(s.unlocked.length/s.items.length*100);return `<button data-category="${cat.id}" class="pillar-card"><span class="pillar-icon">${cat.icon}</span><div><b>${cat.name}</b><small>${s.unlocked.length} / ${s.items.length} developed</small><small class="pillar-source">${readinessSourceLabel(cat.id)}</small><i><u style="width:${pc}%"></u></i></div><em>›</em></button>`}).join('')}</section><section class="classified-tease"><span>☠</span><div><b>CLASSIFIED INTELLIGENCE</b><small>A deeper secret about the enemy remains hidden.</small></div><i>LOCKED</i></section>`;
...
function renderBattle(){const hub=document.querySelector('#battleHub');if(!hub)return;const forecast=battleForecastHtml();const book=battleBook(),boss=BOSS_BATTLES[book],unlockLevel=book===1?20:44;if(save.currentLevel<unlockLevel){hub.innerHTML=forecast+`<section class="battle-cinematic locked-battle"><div class="battle-smoke"></div><span class="eyebrow">NO ACTIVE FINAL BATTLE</span><h2>Prepare for the next trial.</h2><p>Complete your real-world missions to earn XP and build Battle Readiness.</p><button class="primary" data-go="collection">VIEW READINESS</button></section>`;}else{const st=battleState(book),complete=st.done>=st.stages.length,current=st.stages[Math.min(st.done,st.stages.length-1)],pct=complete?100:Math.min(100,Math.round(st.into/current.xp*100));hub.innerHTML=forecast+`<section class="boss-hero book${book}"><span class="eyebrow">${boss.subtitle}</span><h2>${boss.title}</h2><p>${book===1?'Everything Harry learned this year is about to be tested.':'The hidden chamber is open. Preparation becomes survival.'}</p></section><section class="battle-progress-card"><div class="battle-progress-head"><b>BATTLE PROGRESS</b><span>${st.done} / ${st.stages.length}</span></div><div class="battle-segments">${st.stages.map((x,i)=>`<i class="${i<st.done?'done':i===st.done?'current':''}"></i>`).join('')}</div></section>${complete?`<section class="objective-card victory"><span>🏆</span><small>BATTLE COMPLETE</small><h2>${boss.title} DEFEATED</h2><p>Harry survived the final trial and proved this year’s preparation.</p></section>`:`<section class="objective-card"><span class="objective-icon">${current.icon}</span><small>CURRENT OBJECTIVE</small><h2>${current.name}</h2><b>${current.xp} XP REQUIRED</b><div class="objective-meter"><i style="width:${pct}%"></i></div><strong>${st.into} / ${current.xp} XP</strong><em>+${Math.max(0,current.xp-st.into)} XP TO COMPLETE</em><div class="uses-chip">REQUIRED PREPARATION • ${current.uses}</div></section><section class="battle-stage-list">${st.stages.map((x,i)=>`<div class="battle-stage ${i<st.done?'done':i===st.done?'current':'locked'}"><span>${i<st.done?'✓':x.icon}</span><div><b>${x.name}</b><small>${i<st.done?x.reward:i===st.done?'ACTIVE OBJECTIVE':x.xp+' XP'}</small></div></div>`).join('')}</section>`}`;}hub.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>document.querySelector(`.bottom-nav button[data-view="${b.dataset.go}"]`)?.click());}
